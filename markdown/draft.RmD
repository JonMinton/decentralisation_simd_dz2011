...
posterior_summaries %>% 
  ...
  select(period, bound, value) %>% 
  mutate(value = round(value, 3)) %>% 
  spread(bound, value) %>% 
  select(period, median, lower, upper) %>% 
...
This section will present the results of the Dissimilarity index calculations, using the Bayesian modelling approach used in Kavanagh et al 2016. 
...
The overall trend in each place is for RCI to reduce from 2004 to 2012. However within each period the ordering of RCIs between places has not changed, with Dundee showing the most centralisation of poverty, then Aberdeen, and Glasgow, with Edinburgh the most decentralised. In terms of trends over time, Glasgow and Dundee both show steadily declining RCI between SIMD releases, whereas for Aberdeen and Edinbugh RCI values increased from 2004 to 2006, before declining in 2009 and 2012. From 2009 onwards Edinburgh has negative RCI scores, suggesting a peripheralisation rather than centralisation of poverty.
...
## Table of average proportion income deprived by year 
...
```{r id_prop_by_year, eval = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
  simd_2011_reweighted  %>% 
    ...
    select(City = nearest_centre, prop_id, Year = year) %>% 
    spread(Year, prop_id) %>% 
    ungroup() %>% 
    arrange(`2004`) %>% 
