---
title: "The Suburbanisation of Poverty in Scotland’s Cities: Urban Regeneration or Social Cleansing?"
...
simd_2011_reweighted  %>% 
    ...
    mutate(centralness = cut(
      distance_to_centre, 
      breaks = c(0, 8000, 20000, Inf), 
      labels = c("centre", "periphery", "exurb"))
    ) %>% 
  filter(centralness != "exurb") %>% 
...
tmp %>% filter(type == "Income Deprived") %>% 
  ...
  labs(x = "Year", y = "Proportion", title = "Proportion income deprived within 8km of Centre")
...
tmp %>% filter(type == "Not Income Deprived") %>% 
  ...
  geom_line() + geom_point() +
  facet_wrap( ~ City, nrow = 2, scales = "free_y") + 
  labs(x = "Year", y = "Proportion", title = "Proportion Not income deprived within 8km of Centre")
...
[was this done after the previous stage or before? Would have thought before]
